...
function GameDetail({ game, gameStats, playerId, onEntryCreated, onGameUpdated }: {game: Game, gameStats: GameStats|null, playerId: string|null, onEntryCreated: (newEntry: Entry) => void, onGameUpdated: (game: Game) => void}) {
    ...
    return (
        <div className="flex flex-col">
            ...
            {customFieldsList.length > 0 && (
                <section className="flex flex-col mt-6 border border-slate-600 rounded-lg p-4 bg-slate-900/30 backdrop-blur-sm">
                    <Link 
                        to={`/games/${game.id}/statistics${playerId ? `?playerId=${playerId}` : ''}`}
                    ...
                    >
                        ...
                        <span className="text-lg font-medium">View Statistics</span>
                    </Link>
                </section>
            )}
        </div>
